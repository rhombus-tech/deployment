...
impl<F: FieldElement> ExpanderCode<F> {
    ...
    pub fn new(k: usize, expansion_factor: usize) -> Self {
        ...
        let n_initial = k * expansion_factor;
        let n = n_initial.next_power_of_two();
        ...
        for i in 0..k {
            for j in 0..expansion_factor {
                ...
                // Assign field element for expander graph edge
                generator_matrix[i][col] = F::one();
            }
            
            // Ensure each row has exactly expansion_factor non-zero entries (left-regular)
            let mut edges_placed = expansion_factor;
            while edges_placed < expansion_factor {
                let fallback_col = ((i + edges_placed) % n) as usize;
                if generator_matrix[i][fallback_col] == F::zero() {
                    generator_matrix[i][fallback_col] = F::one();
                    edges_placed += 1;
                }
            }
        }
...
impl<F: FieldElement + Send + Sync> LinearCode<F> for ExpanderCode<F> {
    ...
    fn is_systematic(&self) -> bool {
        // Expander codes can be made systematic
        // For simplicity, we assume systematic encoding where
        // the first k symbols of the codeword are the message
        true
    }
}

/// Factory function to create a suitable linear code for WARP
pub fn create_default_linear_code<F: FieldElement + Send + Sync + 'static>(security_parameter: usize) -> Arc<dyn LinearCode<F>> {
