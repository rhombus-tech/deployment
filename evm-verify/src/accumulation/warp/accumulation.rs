...
impl<F: FieldElement> WarpAccumulation<F> {
    ...
    pub fn create_initial_accumulator(&self, witness: &[F]) -> Result<WarpAccumulator<F>, String> {
        ...
        // Create a commitment to the codeword using Merkle tree
        let commitment = self.merkle_commit(&codeword);
    ...
    pub fn accumulate(
        ...
        let pesat = if prev_acc.instance_part.pesat_constraint.is_some() {
            self.generate_pesat_constraint(instance)
        } else {
            None
        };
        
        // Step 4: Create the new accumulator
        let commitment = self.merkle_commit(&batched_codeword);
        let instance_part = AccumulatorInstancePart {
            commitment,
    ...
    pub fn verify(
        ...
        for (idx, value) in &proof.decommitments {
            ...
            // Full Merkle path verification for cryptographic soundness
    ...
    pub fn decide(&self, acc: &WarpAccumulator<F>) -> bool {
        // Check if the accumulated codeword is consistent with
        // the commitment and satisfies all accumulated constraints
        
        // First, verify the codeword commitment is valid
        let expected_commitment = self.merkle_commit(&acc.witness_part.as_ref().unwrap().codeword);
        if acc.instance_part.commitment != expected_commitment {
            return false;
        }
    ...
    /// Get the initial accumulator instance for starting verification
    pub fn get_initial_instance(&self) -> AccumulatorInstancePart<F> {
        // Return an empty initial instance
    ...
    fn merkle_commit(&self, codeword: &[F]) -> Vec<u8> {
        ...
        for elem in codeword {
            ...
            // Hash field element using debug representation
