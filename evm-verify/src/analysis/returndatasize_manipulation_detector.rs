/// Returndatasize Manipulation Detector
use crate::bytecode::SecurityFinding;

pub struct ReturndatasizeManipulationDetector {
    bytecode: Vec<u8>,
}

impl ReturndatasizeManipulationDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Return data size attack at PC {}", location),
                pc: location,
                confidence: 0.84,
            });
        }
