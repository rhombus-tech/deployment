/// Memory Expansion Gas Attack Detector
use crate::bytecode::SecurityFinding;

pub struct MemoryExpansionGasAttackDetector {
    bytecode: Vec<u8>,
}

impl MemoryExpansionGasAttackDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Memory expansion gas griefing at PC {}", location),
                pc: location,
                confidence: 0.82,
            });
        }
