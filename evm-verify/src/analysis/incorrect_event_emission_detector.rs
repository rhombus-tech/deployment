/// Incorrect Event Emission Detector
use crate::bytecode::SecurityFinding;

pub struct IncorrectEventEmissionDetector {
    bytecode: Vec<u8>,
}

impl IncorrectEventEmissionDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Incorrect or missing event data at PC {}", location),
                pc: location,
                confidence: 0.79,
            });
        }
