/// Unchecked Math Assembly Detector
/// Assembly arithmetic without overflow checks
use crate::bytecode::SecurityFinding;

pub struct UncheckedMathAssemblyDetector {
    bytecode: Vec<u8>,
}

impl UncheckedMathAssemblyDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("Assembly arithmetic without overflow checks at PC {}", location),
                pc: location,
                confidence: 0.89,
            });
        }
