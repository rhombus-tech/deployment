/// Storage Layout Incompatibility Detector
use crate::bytecode::SecurityFinding;

pub struct StorageLayoutIncompatibilityDetector {
    bytecode: Vec<u8>,
}

impl StorageLayoutIncompatibilityDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("Storage layout incompatibility in upgrade at PC {}", location),
                pc: location,
                confidence: 0.92,
            });
        }
