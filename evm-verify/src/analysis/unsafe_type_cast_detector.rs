/// Unsafe Type Cast Detector
/// Unsafe type cast causes silent truncation
use crate::bytecode::SecurityFinding;

pub struct UnsafeTypeCastDetector {
    bytecode: Vec<u8>,
}

impl UnsafeTypeCastDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("Unsafe type cast causes silent truncation at PC {}", location),
                pc: location,
                confidence: 0.87,
            });
        }
