/// Bridge Message Spoof Detector
use crate::bytecode::SecurityFinding;

pub struct BridgeMessageSpoofDetector {
    bytecode: Vec<u8>,
}

impl BridgeMessageSpoofDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("Bridge messages can be spoofed at PC {}", location),
                pc: location,
                confidence: 0.93,
            });
        }
