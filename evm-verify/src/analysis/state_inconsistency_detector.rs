/// State Inconsistency Detector
/// State inconsistency between storage variables
...
impl StateInconsistencyDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("State inconsistency between storage variables at PC {}", location),
                pc: location,
                confidence: 0.88,
            });
        }
