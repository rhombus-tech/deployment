/// Blast Points Farming Detector
use crate::bytecode::SecurityFinding;

pub struct BlastPointsFarmingDetector {
    bytecode: Vec<u8>,
}

impl BlastPointsFarmingDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_exploit_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Blast points farming exploit at PC {}", location),
                pc: location,
                confidence: 0.87,
            });
        }
