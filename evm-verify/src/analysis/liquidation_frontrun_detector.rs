/// Liquidation Frontrun Detector
use crate::bytecode::SecurityFinding;

pub struct LiquidationFrontrunDetector {
    bytecode: Vec<u8>,
}

impl LiquidationFrontrunDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Liquidation frontrunning at PC {}", location),
                pc: location,
                confidence: 0.89,
            });
        }
