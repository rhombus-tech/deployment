/// Pool Donation Attack Detector
use crate::bytecode::SecurityFinding;

pub struct PoolDonationAttackDetector {
    bytecode: Vec<u8>,
}

impl PoolDonationAttackDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Pool donation inflates share price at PC {}", location),
                pc: location,
                confidence: 0.93,
            });
        }
