/// Calldata Size Manipulation Detector
use crate::bytecode::SecurityFinding;

pub struct CalldataSizeManipulationDetector {
    bytecode: Vec<u8>,
}

impl CalldataSizeManipulationDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Calldata size not validated at PC {}", location),
                pc: location,
                confidence: 0.83,
            });
        }
