/// Qubit Bridge Safetransferfrom Detector
use crate::bytecode::SecurityFinding;

pub struct QubitBridgeSafetransferfromDetector {
    bytecode: Vec<u8>,
}

impl QubitBridgeSafetransferfromDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Qubit bridge safeTransferFrom exploit at PC {}", location),
                pc: location,
                confidence: 0.91,
            });
        }
