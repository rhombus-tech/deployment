/// Wrong Comparison Operator Detector
use crate::bytecode::SecurityFinding;

pub struct WrongComparisonOperatorDetector {
    bytecode: Vec<u8>,
}

impl WrongComparisonOperatorDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Wrong comparison operator (< vs <=) at PC {}", location),
                pc: location,
                confidence: 0.85,
            });
        }
