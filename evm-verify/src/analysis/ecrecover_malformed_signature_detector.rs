/// Ecrecover Malformed Signature Detector
use crate::bytecode::SecurityFinding;

pub struct EcrecoverMalformedSignatureDetector {
    bytecode: Vec<u8>,
}

impl EcrecoverMalformedSignatureDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Ecrecover with malformed signature at PC {}", location),
                pc: location,
                confidence: 0.88,
            });
        }
