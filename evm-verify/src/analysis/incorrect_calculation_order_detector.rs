/// Incorrect Calculation Order Detector
use crate::bytecode::SecurityFinding;

pub struct IncorrectCalculationOrderDetector {
    bytecode: Vec<u8>,
}

impl IncorrectCalculationOrderDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Wrong calculation order at PC {}", location),
                pc: location,
                confidence: 0.83,
            });
        }
