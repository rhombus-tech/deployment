/// Aave Health Factor Detector
use crate::bytecode::SecurityFinding;

pub struct AaveHealthFactorDetector {
    bytecode: Vec<u8>,
}

impl AaveHealthFactorDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Aave health factor manipulation at PC {}", location),
                pc: location,
                confidence: 0.87,
            });
        }
