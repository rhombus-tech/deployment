/// Forced Inclusion Griefing Detector
use crate::bytecode::SecurityFinding;

pub struct ForcedInclusionGriefingDetector {
    bytecode: Vec<u8>,
}

impl ForcedInclusionGriefingDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.has_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("Forced inclusion griefing detected at PC {}", location),
                pc: location,
                confidence: 0.82,
            });
        }
    ...
    fn has_vulnerability(&self) -> Option<usize> {
        for i in 0..self.bytecode.len().saturating_sub(50) {
            ...
            // Real bytecode pattern detection for forceInclusion and 42
