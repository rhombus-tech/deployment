/// Modulo Zero Division Detector
/// Modulo by zero causes revert
use crate::bytecode::SecurityFinding;

pub struct ModuloZeroDivisionDetector {
    bytecode: Vec<u8>,
}

impl ModuloZeroDivisionDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("Modulo by zero causes revert at PC {}", location),
                pc: location,
                confidence: 0.94,
            });
        }
