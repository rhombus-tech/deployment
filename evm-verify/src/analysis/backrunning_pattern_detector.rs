/// Backrunning Pattern Detector
use crate::bytecode::SecurityFinding;

pub struct BackrunningPatternDetector {
    bytecode: Vec<u8>,
}

impl BackrunningPatternDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Backrunning MEV pattern at PC {}", location),
                pc: location,
                confidence: 0.83,
            });
        }
