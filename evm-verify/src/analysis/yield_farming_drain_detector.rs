/// Yield Farming Drain Detector
use crate::bytecode::SecurityFinding;

pub struct YieldFarmingDrainDetector {
    bytecode: Vec<u8>,
}

impl YieldFarmingDrainDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Yield farming drain attack at PC {}", location),
                pc: location,
                confidence: 0.88,
            });
        }
