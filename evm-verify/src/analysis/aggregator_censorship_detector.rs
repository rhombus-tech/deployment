/// Aggregator Censorship Detector
use crate::bytecode::SecurityFinding;

pub struct AggregatorCensorshipDetector {
    bytecode: Vec<u8>,
}

impl AggregatorCensorshipDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("Aggregator can censor user operations at PC {}", location),
                pc: location,
                confidence: 0.82,
            });
        }
