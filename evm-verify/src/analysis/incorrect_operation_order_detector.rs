/// Incorrect Operation Order Detector
use crate::bytecode::SecurityFinding;

pub struct IncorrectOperationOrderDetector {
    bytecode: Vec<u8>,
}

impl IncorrectOperationOrderDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Incorrect operation order at PC {}", location),
                pc: location,
                confidence: 0.82,
            });
        }
