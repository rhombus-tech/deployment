/// Merkle Proof Collision Detector
use crate::bytecode::SecurityFinding;

pub struct MerkleProofCollisionDetector {
    bytecode: Vec<u8>,
}

impl MerkleProofCollisionDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_exploit_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Merkle airdrop proof collision at PC {}", location),
                pc: location,
                confidence: 0.89,
            });
        }
