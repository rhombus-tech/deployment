/// Multichain Anyswap Permit Detector
use crate::bytecode::SecurityFinding;

pub struct MultichainAnyswapPermitDetector {
    bytecode: Vec<u8>,
}

impl MultichainAnyswapPermitDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Multichain AnySwap permit vulnerability at PC {}", location),
                pc: location,
                confidence: 0.88,
            });
        }
