/// Unrestricted Approval Detector
use crate::bytecode::SecurityFinding;

pub struct UnrestrictedApprovalDetector {
    bytecode: Vec<u8>,
}

impl UnrestrictedApprovalDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Unlimited approval without validation at PC {}", location),
                pc: location,
                confidence: 0.88,
            });
        }
