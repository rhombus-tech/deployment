/// Staticcall State Change Detection Detector
use crate::bytecode::SecurityFinding;

pub struct StaticcallStateChangeDetectionDetector {
    bytecode: Vec<u8>,
}

impl StaticcallStateChangeDetectionDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("STATICCALL modifying state at PC {}", location),
                pc: location,
                confidence: 0.87,
            });
        }
