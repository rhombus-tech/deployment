/// Paymaster Griefing Detector
use crate::bytecode::SecurityFinding;

pub struct PaymasterGriefingDetector {
    bytecode: Vec<u8>,
}

impl PaymasterGriefingDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("Paymaster can be griefed draining funds at PC {}", location),
                pc: location,
                confidence: 0.88,
            });
        }
