/// Slippage Frontrun Detector
use crate::bytecode::SecurityFinding;

pub struct SlippageFrontrunDetector {
    bytecode: Vec<u8>,
}

impl SlippageFrontrunDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Slippage frontrunning attack at PC {}", location),
                pc: location,
                confidence: 0.88,
            });
        }
