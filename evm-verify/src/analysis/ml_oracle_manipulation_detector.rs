/// Ml Oracle Manipulation Detector
use crate::bytecode::SecurityFinding;

pub struct MlOracleManipulationDetector {
    bytecode: Vec<u8>,
}

impl MlOracleManipulationDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_exploit_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("ML oracle manipulation at PC {}", location),
                pc: location,
                confidence: 0.86,
            });
        }
