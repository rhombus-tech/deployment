/// Nonce Key Collision Aa Detector
use crate::bytecode::SecurityFinding;

pub struct NonceKeyCollisionAaDetector {
    bytecode: Vec<u8>,
}

impl NonceKeyCollisionAaDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_exploit_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("AA nonce key collision at PC {}", location),
                pc: location,
                confidence: 0.86,
            });
        }
