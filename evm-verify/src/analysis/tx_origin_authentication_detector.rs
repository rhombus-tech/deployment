...
/// tx.origin authentication bypass allows phishing attacks
use crate::bytecode::SecurityFinding;

pub struct TxOriginAuthenticationDetector {
    bytecode: Vec<u8>,
}

impl TxOriginAuthenticationDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("tx.origin authentication bypass allows phishing attacks at PC {}", location),
                pc: location,
                confidence: 0.89,
            });
        }
