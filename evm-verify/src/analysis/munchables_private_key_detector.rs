/// Munchables Private Key Detector
use crate::bytecode::SecurityFinding;

pub struct MunchablesPrivateKeyDetector {
    bytecode: Vec<u8>,
}

impl MunchablesPrivateKeyDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_exploit_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Private key exposure in contract logic at PC {}", location),
                pc: location,
                confidence: 0.95,
            });
        }
