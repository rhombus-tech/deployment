/// Withdraw Reentrancy Attack Detector
use crate::bytecode::SecurityFinding;

pub struct WithdrawReentrancyAttackDetector {
    bytecode: Vec<u8>,
}

impl WithdrawReentrancyAttackDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Reentrancy during withdrawal at PC {}", location),
                pc: location,
                confidence: 0.95,
            });
        }
