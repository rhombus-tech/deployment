/// Vault Inflation Attack Detector
use crate::bytecode::SecurityFinding;

pub struct VaultInflationAttackDetector {
    bytecode: Vec<u8>,
}

impl VaultInflationAttackDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("ERC4626 vault inflation via donation at PC {}", location),
                pc: location,
                confidence: 0.92,
            });
        }
