/// Paymaster Drain Attack Detector
use crate::bytecode::SecurityFinding;

pub struct PaymasterDrainAttackDetector {
    bytecode: Vec<u8>,
}

impl PaymasterDrainAttackDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_exploit_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Paymaster fund draining attack at PC {}", location),
                pc: location,
                confidence: 0.92,
            });
        }
