/// Balancer Pool Tokens Detector
use crate::bytecode::SecurityFinding;

pub struct BalancerPoolTokensDetector {
    bytecode: Vec<u8>,
}

impl BalancerPoolTokensDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Balancer pool token manipulation at PC {}", location),
                pc: location,
                confidence: 0.83,
            });
        }
