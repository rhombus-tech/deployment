/// Restaking Slashing Detector
use crate::bytecode::SecurityFinding;

pub struct RestakingSlashingDetector {
    bytecode: Vec<u8>,
}

impl RestakingSlashingDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_exploit_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Restaking slashing vulnerability at PC {}", location),
                pc: location,
                confidence: 0.88,
            });
        }
