/// Out Of Gas Revert Detector
use crate::bytecode::SecurityFinding;

pub struct OutOfGasRevertDetector {
    bytecode: Vec<u8>,
}

impl OutOfGasRevertDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Function always runs out of gas at PC {}", location),
                pc: location,
                confidence: 0.81,
            });
        }
