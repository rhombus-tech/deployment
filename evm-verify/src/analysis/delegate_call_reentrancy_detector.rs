/// Delegate Call Reentrancy Detector
/// Delegatecall reentrancy enables context hijacking
...
impl DelegateCallReentrancyDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("Delegatecall reentrancy enables context hijacking at PC {}", location),
                pc: location,
                confidence: 0.94,
            });
        }
