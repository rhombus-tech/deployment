/// Bribing Attack Detection Detector
use crate::bytecode::SecurityFinding;

pub struct BribingAttackDetectionDetector {
    bytecode: Vec<u8>,
}

impl BribingAttackDetectionDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_exploit_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Vote bribing vulnerability at PC {}", location),
                pc: location,
                confidence: 0.83,
            });
        }
