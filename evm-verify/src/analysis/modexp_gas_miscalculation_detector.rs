/// Modexp Gas Miscalculation Detector
use crate::bytecode::SecurityFinding;

pub struct ModexpGasMiscalculationDetector {
    bytecode: Vec<u8>,
}

impl ModexpGasMiscalculationDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("MODEXP gas calculation error at PC {}", location),
                pc: location,
                confidence: 0.81,
            });
        }
