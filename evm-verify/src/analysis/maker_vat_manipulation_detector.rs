/// Maker Vat Manipulation Detector
use crate::bytecode::SecurityFinding;

pub struct MakerVatManipulationDetector {
    bytecode: Vec<u8>,
}

impl MakerVatManipulationDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("MakerDAO Vat manipulation at PC {}", location),
                pc: location,
                confidence: 0.9,
            });
        }
