/// Balancer Swap Fee Detector
use crate::bytecode::SecurityFinding;

pub struct BalancerSwapFeeDetector {
    bytecode: Vec<u8>,
}

impl BalancerSwapFeeDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Balancer swap fee manipulation at PC {}", location),
                pc: location,
                confidence: 0.82,
            });
        }
