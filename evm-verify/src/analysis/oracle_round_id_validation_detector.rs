/// Oracle Round Id Validation Detector
use crate::bytecode::SecurityFinding;

pub struct OracleRoundIdValidationDetector {
    bytecode: Vec<u8>,
}

impl OracleRoundIdValidationDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Oracle round ID not validated at PC {}", location),
                pc: location,
                confidence: 0.87,
            });
        }
