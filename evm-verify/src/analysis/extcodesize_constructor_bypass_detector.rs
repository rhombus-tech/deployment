/// Extcodesize Constructor Bypass Detector
use crate::bytecode::SecurityFinding;

pub struct ExtcodesizeConstructorBypassDetector {
    bytecode: Vec<u8>,
}

impl ExtcodesizeConstructorBypassDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("EXTCODESIZE bypass in constructor at PC {}", location),
                pc: location,
                confidence: 0.89,
            });
        }
