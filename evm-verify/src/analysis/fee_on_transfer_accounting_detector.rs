/// Fee On Transfer Accounting Detector
/// Fee-on-transfer token accounting mismatch
use crate::bytecode::SecurityFinding;

pub struct FeeOnTransferAccountingDetector {
    bytecode: Vec<u8>,
}

impl FeeOnTransferAccountingDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("Fee-on-transfer token accounting mismatch at PC {}", location),
                pc: location,
                confidence: 0.88,
            });
        }
