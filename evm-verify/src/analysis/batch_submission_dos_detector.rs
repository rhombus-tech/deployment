/// Batch Submission Dos Detector
use crate::bytecode::SecurityFinding;

pub struct BatchSubmissionDosDetector {
    bytecode: Vec<u8>,
}

impl BatchSubmissionDosDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.has_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("Batch submission DOS detected at PC {}", location),
                pc: location,
                confidence: 0.86,
            });
        }
    ...
    fn has_vulnerability(&self) -> Option<usize> {
        for i in 0..self.bytecode.len().saturating_sub(50) {
            ...
            // Real bytecode pattern detection for submitBatch and gas
