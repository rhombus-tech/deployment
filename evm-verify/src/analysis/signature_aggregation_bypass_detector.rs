/// Signature Aggregation Bypass Detector
use crate::bytecode::SecurityFinding;

pub struct SignatureAggregationBypassDetector {
    bytecode: Vec<u8>,
}

impl SignatureAggregationBypassDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_exploit_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Signature aggregation bypass at PC {}", location),
                pc: location,
                confidence: 0.87,
            });
        }
