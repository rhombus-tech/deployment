/// Scroll Compressed Calldata Detector
use crate::bytecode::SecurityFinding;

pub struct ScrollCompressedCalldataDetector {
    bytecode: Vec<u8>,
}

impl ScrollCompressedCalldataDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_exploit_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Scroll L2 calldata compression issues at PC {}", location),
                pc: location,
                confidence: 0.82,
            });
        }
