/// Abi Encodepacked Collision Detector
use crate::bytecode::SecurityFinding;

pub struct AbiEncodepackedCollisionDetector {
    bytecode: Vec<u8>,
}

impl AbiEncodepackedCollisionDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("abi.encodePacked hash collision at PC {}", location),
                pc: location,
                confidence: 0.86,
            });
        }
