/// External Call Gas Griefing Detector
use crate::bytecode::SecurityFinding;

pub struct ExternalCallGasGriefingDetector {
    bytecode: Vec<u8>,
}

impl ExternalCallGasGriefingDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Insufficient gas for external calls at PC {}", location),
                pc: location,
                confidence: 0.84,
            });
        }
