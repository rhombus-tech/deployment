...
pub enum WithdrawalDelayVulnerability {
...
impl WithdrawalDelayDetector {
    ...
    pub fn detect_vulnerabilities(&self) -> Vec<WithdrawalDelayVulnerability> {
        let mut vulnerabilities = Vec::new();
        
        // Pattern-based detection for withdrawal delay
        for i in 0..self.bytecode.len().saturating_sub(20) {
            // Check for suspicious patterns
            if self.bytecode[i] == 0xf1 || // CALL
               self.bytecode[i] == 0xf4 || // DELEGATECALL
               self.bytecode[i] == 0x55 {  // SSTORE
                vulnerabilities.push(WithdrawalDelayVulnerability::Medium {
