...
pub struct EcdsaSignatureMalleabilityDetector {
    bytecode: Vec<u8>,
}

impl EcdsaSignatureMalleabilityDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.has_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("ECDSA malleability detected at PC {}", location),
                pc: location,
                confidence: 0.88,
            });
        }
    ...
    fn has_vulnerability(&self) -> Option<usize> {
        for i in 0..self.bytecode.len().saturating_sub(50) {
            ...
            // Real bytecode pattern detection for ecrecover and s_value
