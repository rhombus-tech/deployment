/// Hundred Finance Chainlink Detector
use crate::bytecode::SecurityFinding;

pub struct HundredFinanceChainlinkDetector {
    bytecode: Vec<u8>,
}

impl HundredFinanceChainlinkDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Hundred Finance oracle manipulation at PC {}", location),
                pc: location,
                confidence: 0.87,
            });
        }
