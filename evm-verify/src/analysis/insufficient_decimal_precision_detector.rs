/// Insufficient Decimal Precision Detector
use crate::bytecode::SecurityFinding;

pub struct InsufficientDecimalPrecisionDetector {
    bytecode: Vec<u8>,
}

impl InsufficientDecimalPrecisionDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Decimal precision mismatch in price feeds at PC {}", location),
                pc: location,
                confidence: 0.88,
            });
        }
