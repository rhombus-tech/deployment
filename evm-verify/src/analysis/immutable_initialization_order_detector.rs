/// Immutable Initialization Order Detector
use crate::bytecode::SecurityFinding;

pub struct ImmutableInitializationOrderDetector {
    bytecode: Vec<u8>,
}

impl ImmutableInitializationOrderDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Immutable variable init order bug at PC {}", location),
                pc: location,
                confidence: 0.83,
            });
        }
