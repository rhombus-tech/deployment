/// Sentiment Reentrancy Redeem Detector
use crate::bytecode::SecurityFinding;

pub struct SentimentReentrancyRedeemDetector {
    bytecode: Vec<u8>,
}

impl SentimentReentrancyRedeemDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Sentiment reentrancy in redeem at PC {}", location),
                pc: location,
                confidence: 0.92,
            });
        }
