/// Delegatecall To Arbitrary Detector
use crate::bytecode::SecurityFinding;

pub struct DelegatecallToArbitraryDetector {
    bytecode: Vec<u8>,
}

impl DelegatecallToArbitraryDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("Delegatecall to arbitrary address at PC {}", location),
                pc: location,
                confidence: 0.94,
            });
        }
