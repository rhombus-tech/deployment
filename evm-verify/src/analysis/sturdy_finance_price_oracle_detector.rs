/// Sturdy Finance Price Oracle Detector
use crate::bytecode::SecurityFinding;

pub struct SturdyFinancePriceOracleDetector {
    bytecode: Vec<u8>,
}

impl SturdyFinancePriceOracleDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_exploit_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Sturdy Finance price oracle manipulation at PC {}", location),
                pc: location,
                confidence: 0.9,
            });
        }
