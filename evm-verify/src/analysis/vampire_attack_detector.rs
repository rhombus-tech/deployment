...
pub enum VampireAttackVulnerability {
...
impl VampireAttackDetector {
    ...
    pub fn detect_vulnerabilities(&self) -> Vec<VampireAttackVulnerability> {
        let mut vulnerabilities = Vec::new();
        
        // Pattern-based detection for vampire attack
        for i in 0..self.bytecode.len().saturating_sub(20) {
            // Check for suspicious patterns
            if self.bytecode[i] == 0xf1 || // CALL
               self.bytecode[i] == 0xf4 || // DELEGATECALL
               self.bytecode[i] == 0x55 {  // SSTORE
                vulnerabilities.push(VampireAttackVulnerability::Medium {
