/// Donation Inflation Detector
use crate::bytecode::SecurityFinding;

pub struct DonationInflationDetector {
    bytecode: Vec<u8>,
}

impl DonationInflationDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Donation attack inflates share price at PC {}", location),
                pc: location,
                confidence: 0.92,
            });
        }
