/// Nft Offer Griefing Detector
use crate::bytecode::SecurityFinding;

pub struct NftOfferGriefingDetector {
    bytecode: Vec<u8>,
}

impl NftOfferGriefingDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.has_vulnerability() {
            findings.push(SecurityFinding {
                severity: crate::bytecode::SecuritySeverity::Low,
                description: format!("NFT offer griefing attack detected at PC {}", location),
                pc: location,
                confidence: 0.78,
            });
        }
    ...
    fn has_vulnerability(&self) -> Option<usize> {
        for i in 0..self.bytecode.len().saturating_sub(50) {
            ...
            // Real bytecode pattern detection for accept_offer and fd
