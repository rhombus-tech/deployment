/// Incorrect Comparison Operator Detector
use crate::bytecode::SecurityFinding;

pub struct IncorrectComparisonOperatorDetector {
    bytecode: Vec<u8>,
}

impl IncorrectComparisonOperatorDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Incorrect comparison operator at PC {}", location),
                pc: location,
                confidence: 0.84,
            });
        }
