/// Proposer Boost Gaming Detector
use crate::bytecode::SecurityFinding;

pub struct ProposerBoostGamingDetector {
    bytecode: Vec<u8>,
}

impl ProposerBoostGamingDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.has_vulnerability() {
            findings.push(SecurityFinding {
                ...
                description: format!("Proposer boost gaming detected at PC {}", location),
                pc: location,
                confidence: 0.81,
            });
        }
    ...
    fn has_vulnerability(&self) -> Option<usize> {
        for i in 0..self.bytecode.len().saturating_sub(50) {
            ...
            // Real bytecode pattern detection for propose and timestamp
