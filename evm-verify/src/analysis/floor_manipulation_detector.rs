/// Floor Manipulation Detector
use crate::bytecode::SecurityFinding;

pub struct FloorManipulationDetector {
    bytecode: Vec<u8>,
}

impl FloorManipulationDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("NFT floor price manipulation at PC {}", location),
                pc: location,
                confidence: 0.83,
            });
        }
