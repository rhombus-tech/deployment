/// Array Deletion Gap Detector
use crate::bytecode::SecurityFinding;

pub struct ArrayDeletionGapDetector {
    bytecode: Vec<u8>,
}

impl ArrayDeletionGapDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Array deletion leaves gaps at PC {}", location),
                pc: location,
                confidence: 0.83,
            });
        }
