/// Price Staleness Check Detector
use crate::bytecode::SecurityFinding;

pub struct PriceStalenessCheckDetector {
    bytecode: Vec<u8>,
}

impl PriceStalenessCheckDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Stale oracle price not validated at PC {}", location),
                pc: location,
                confidence: 0.91,
            });
        }
