/// Chainlink L2 Sequencer Check Detector
use crate::bytecode::SecurityFinding;

pub struct ChainlinkL2SequencerCheckDetector {
    bytecode: Vec<u8>,
}

impl ChainlinkL2SequencerCheckDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("L2 sequencer downtime not checked at PC {}", location),
                pc: location,
                confidence: 0.9,
            });
        }
