/// Share Price Inflation Attack Detector
use crate::bytecode::SecurityFinding;

pub struct SharePriceInflationAttackDetector {
    bytecode: Vec<u8>,
}

impl SharePriceInflationAttackDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("First depositor attack inflates shares at PC {}", location),
                pc: location,
                confidence: 0.94,
            });
        }
