/// Optimism Mint Inflation Detector
use crate::bytecode::SecurityFinding;

pub struct OptimismMintInflationDetector {
    bytecode: Vec<u8>,
}

impl OptimismMintInflationDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_exploit_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Optimism bridge mint inflation at PC {}", location),
                pc: location,
                confidence: 0.93,
            });
        }
