/// Cream Finance Reentrancy Detector
use crate::bytecode::SecurityFinding;

pub struct CreamFinanceReentrancyDetector {
    bytecode: Vec<u8>,
}

impl CreamFinanceReentrancyDetector {
    ...
    pub fn detect(&self) -> Vec<SecurityFinding> {
        ...
        if let Some(location) = self.detect_pattern() {
            findings.push(SecurityFinding {
                ...
                description: format!("Cream Finance reentrancy pattern at PC {}", location),
                pc: location,
                confidence: 0.93,
            });
        }
