...
# Find all helper method definitions (fn has_*)
methods = {}
for i, line in enumerate(lines):
    match = re.search(r'fn (has_\w+)\(&self', line)
    if match:
...
print(f"Found {len(duplicates)} helper methods with duplicates:")
...
for name, locs in duplicates.items():
    ...
    for loc in locs[1:]:
        # Find the complete method (from fn to closing brace)
        start = loc
        end = loc + 1
        brace_count = 0
        started = False
        for j in range(loc, min(loc + 30, len(lines))):
            if '{' in lines[j]:
                started = True
...
print("Done! Helper duplicates removed.")
