...
fn main() -> Result<()> {
    ...
    match verifier.generate_pcc_proof(bytecode_bytes.as_ref()) {
        Ok(proof) => {
            println!("PCC proof generated successfully!");
            
            // Verify PCC proof
            println!("Verifying PCC proof...");
            match verifier.verify_pcc_proof(bytecode_bytes.as_ref(), proof.as_ref()) {
                Ok(result) => {
                    println!("PCC proof verification result: {}", if result.is_valid { "Valid" } else { "Invalid" });
                },
                Err(e) => {
    ...
    match verifier.generate_pcd_proof(bytecode_bytes.as_ref()) {
        Ok((proof, verifying_key)) => {
            ...
            match verifier.verify_pcd_proof(bytecode_bytes.as_ref(), proof.as_ref(), verifying_key.as_ref()) {
                Ok(result) => {
                    println!("PCD proof verification result: {}", if result.is_valid { "Valid" } else { "Invalid" });
                },
                Err(e) => {
