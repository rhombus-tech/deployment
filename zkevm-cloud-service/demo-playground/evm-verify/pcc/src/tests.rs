...
fn test_mev_vulnerability_detection() {
    ...
    use ark_relations::r1cs::ConstraintSystem;
    use ethers::types::U256;
    // Sample bytecode with common MEV vulnerability patterns
    // This includes price oracle manipulation and front-running vectors
    const MEV_VULNERABLE_BYTECODE: &[u8] = &[
        // PUSH oracle address and CALL pattern
        0x73, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, // PUSH20 oracle_address
        0x63, 0x41, 0x4b, 0xf3, 0x89, // PUSH4 oracle_function_selector (getPrice)
        0x5A, // GAS
        0xF1, // CALL
        
        // tx.origin check - front-running vector
        0x32, // ORIGIN
        0x73, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0x00, 0xaa, 0xbb, 0xcc, 0xdd, // PUSH20 owner_address
        0x14, // EQ
        0x60, 0x01, // PUSH1 0x01
        0x57, // JUMPI
        ...
        // Insufficient slippage protection pattern
        0x60, 0x00, // PUSH1 0x00 (min_amount = 0)
        0x35, // CALLDATALOAD
        0x60, 0x01, // PUSH1 0x01
        0x01, // ADD
        0x90, // SWAP1
    ...
    // Create a circuit with the vulnerable bytecode
    // Using proper constructor with all required parameters
    let circuit = BytecodeSafetyCircuit::<Fr>::new(
        &[VulnerabilityType::MevVulnerability], 
        U256::from(21000),         // gas_usage
        10,                         // complexity
        MEV_VULNERABLE_BYTECODE.to_vec(), // bytecode
        None                        // bytecode_hash
    );
    ...
    // Run the MEV vulnerability detection and formal verification
    let result = circuit.verify_mev_vulnerability(&mut cs.clone());
    assert!(result.is_ok(), "MEV vulnerability verification failed");
    
    // Also test the sandwich attack detection
    let sandwich_result = circuit.verify_sandwich_attack(&mut cs.clone());
    assert!(sandwich_result.is_ok(), "Sandwich attack verification failed");
    
    // Check that constraints were generated
    let num_constraints = cs.num_constraints();
    
    println!("Generated {} constraints for MEV vulnerability verification", num_constraints);
    assert!(num_constraints > 0, "No constraints were generated for MEV verification");
    
    // Verify that the circuit is satisfied
    let is_satisfied = cs.is_satisfied().unwrap_or(false);
    println!("Circuit is satisfied: {}", is_satisfied);
    
    // Generate a proving key to ensure the circuit compiles
    // Calling directly from crate root as imported in the use statements
    let pk_result = crate::prover::generate_proving_key(&circuit);
    assert!(pk_result.is_ok(), "Failed to generate proving key for MEV vulnerability circuit");
    
    println!("MEV vulnerability detection and formal verification test passed!");
}

#[test]
fn test_time_bandit_vulnerability_detection() {
    use crate::circuits::bytecode::BytecodeSafetyCircuit;
    use crate::analyzer::bytecode::VulnerabilityType;
    use ark_bn254::Fr;
    use ark_relations::r1cs::ConstraintSystem;
    ...
    // Sample bytecode with time-bandit vulnerability patterns:
    // - Timestamp dependency (TIMESTAMP opcode)
    // - High-value transfers (CALL with value)
    // - No block number checks (missing NUMBER opcode)
    const TIME_BANDIT_BYTECODE: &[u8] = &[
        // Timestamp dependency without block number check
        0x42, // TIMESTAMP
        0x60, 0x05, // PUSH1 0x05
        0x01, // ADD
        0x60, 0x00, // PUSH1 0x00
        ...
        // High-value transfer (CALL with value)
        0x73, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, // PUSH20 target_address
        0x67, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // PUSH8 high_value
        0x60, 0x00, // PUSH1 0x00 (no data)
        0x60, 0x00, // PUSH1 0x00 (no data length)
        0x60, 0x00, // PUSH1 0x00 (no return data)
        0x60, 0x00, // PUSH1 0x00 (no return data length)
        0xf1, // CALL
        
        // Decision logic based on timestamp
        0x60, 0x05, // PUSH1 0x05
        0x42, // TIMESTAMP
        0x10, // LT
        0x60, 0x01, // PUSH1 0x01
    ...
    // Create a circuit with the time-bandit vulnerable bytecode
    let circuit = BytecodeSafetyCircuit::<Fr>::new(
        &[VulnerabilityType::MevVulnerability], 
        U256::from(21000),
        10,
        TIME_BANDIT_BYTECODE.to_vec(),
        None
    );
    
    let cs = ConstraintSystem::<Fr>::new_ref();
    
    // Verify the time-bandit vulnerability detection
    let result = circuit.verify_mev_vulnerability(&mut cs.clone());
    assert!(result.is_ok(), "Time-bandit vulnerability verification failed");
    
    // Ensure constraints were generated
    assert!(cs.num_constraints() > 0, "No constraints were generated for time-bandit test");
}

#[test]
fn test_oracle_manipulation_vulnerability_detection() {
    use crate::circuits::bytecode::BytecodeSafetyCircuit;
    use crate::analyzer::bytecode::VulnerabilityType;
    use ark_bn254::Fr;
    use ark_relations::r1cs::ConstraintSystem;
    ...
    // Sample bytecode with oracle manipulation vulnerability patterns:
    // - Single oracle source (one CALL to oracle)
    // - No staleness check
    // - No TWAP mechanism
    const ORACLE_MANIPULATION_BYTECODE: &[u8] = &[
        // Single oracle call
        0x73, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x12, 0x34, 0x56, // PUSH20 oracle_address
        0x63, 0x41, 0x4b, 0xf3, 0x89, // PUSH4 oracle_function_selector (getPrice)
        0x5A, // GAS
        ...
        // Use price without checks
        0x60, 0x00, // PUSH1 0x00
        0x51, // MLOAD (load oracle result)
        
        // Simple swap logic using oracle price
        0x90, // SWAP1
        0x04, // DIV (divide by price)
        0x60, 0x00, // PUSH1 0x00
        0x53, // MSTORE8
        
        // No timestamp comparison or staleness check
        
        // No multiple oracle sources or averaging logic
    ];
    
    // Create a circuit with the oracle manipulation vulnerable bytecode
    let circuit = BytecodeSafetyCircuit::<Fr>::new(
        &[VulnerabilityType::MevVulnerability], 
        U256::from(21000),
        10,
        ORACLE_MANIPULATION_BYTECODE.to_vec(),
        None
    );
    
    let cs = ConstraintSystem::<Fr>::new_ref();
    
    // Verify the oracle manipulation vulnerability detection
    let result = circuit.verify_mev_vulnerability(&mut cs.clone());
    assert!(result.is_ok(), "Oracle manipulation vulnerability verification failed");
    
    // Ensure constraints were generated
    assert!(cs.num_constraints() > 0, "No constraints were generated for oracle manipulation test");
}
