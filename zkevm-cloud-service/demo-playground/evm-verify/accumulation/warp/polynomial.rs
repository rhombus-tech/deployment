...
impl MultilinearPolynomial {
    ...
    pub fn sum_evaluations(&self) -> WarpField {
        self.evaluations.values().fold(WarpField::zero(), |acc, &val| acc.add(&val))
    }
    ...
    pub fn restrict_variable(&self, var_idx: usize, value: bool) -> Self {
        ...
        for point in 0..expected_size {
            let mut full_point = point;
            
            // Insert the fixed value for the specified variable
            if value {
                full_point |= 1 << var_idx;
            }
            
            // Adjust indices that are beyond the fixed variable
            let mut adjusted_full_point = 0;
            for i in 0..self.num_vars {
                if i == var_idx {
                    continue;
                }
                
                let new_i = if i < var_idx { i } else { i - 1 };
                if ((full_point >> i) & 1) == 1 {
                    adjusted_full_point |= 1 << new_i;
                }
            }
            
            // Copy the evaluation to the new polynomial
            let eval = self.evaluations.get(&full_point).unwrap_or(&WarpField::zero());
            new_evaluations.insert(adjusted_full_point, *eval);
        }
...
impl Add for MultilinearPolynomial {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        ...
        for (point, value) in self.evaluations {
            let rhs_value = rhs.evaluations.get(&point).unwrap_or(&WarpField::zero());
            result.evaluations.insert(point, value.add(rhs_value));
        }
...
impl Sub for MultilinearPolynomial {
    ...
    fn sub(self, rhs: Self) -> Self::Output {
        ...
        for (point, value) in self.evaluations {
            let rhs_value = rhs.evaluations.get(&point).unwrap_or(&WarpField::zero());
            result.evaluations.insert(point, value.sub(rhs_value));
        }
...
impl Mul for MultilinearPolynomial {
    ...
    fn mul(self, rhs: Self) -> Self::Output {
        ...
        for (point, value) in self.evaluations {
            let rhs_value = rhs.evaluations.get(&point).unwrap_or(&WarpField::zero());
            result.evaluations.insert(point, value.mul(rhs_value));
        }
        ...
        for idx in 0..(1 << self.num_vars) {
            ...
            for i in 0..self.num_vars {
                ...
                let term = if bit {
                    ...
                    // (1 - r_i) (when x_i = 0)
                    WarpField::one().sub(&r_i)
                };
                
                weight = weight.mul(&term);
            }
            ...
            result = result.add(&fx.mul(&weight));
        }
...
mod tests {
    ...
    fn test_multilinear_extension() {
        ...
        let half = WarpField::from(1u64).inverse().unwrap().mul(&WarpField::from(2u64));  // 1/2
        let point = [half, half];
        
        // Expected result: (1/2)*(1/2)*1 + (1/2)*(1/2)*2 + (1/2)*(1/2)*3 + (1/2)*(1/2)*4 = 2.5
        let expected = WarpField::from(5u64).mul(&half);  // 5/2
