...
#[tokio::test]
async fn test_bytecode_verification() {
    ...
    // Analyze bytecode
    let report_result = verifier.analyze_bytecode(reentrancy_bytes.as_ref()).await;
    ...
    // Analyze bytecode
    let report_result = verifier.analyze_bytecode(safe_bytes.as_ref()).await;
