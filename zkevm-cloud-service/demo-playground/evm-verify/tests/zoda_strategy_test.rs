...
async fn test_zoda_strategy() -> Result<()> {
    ...
    // Verify that we have reentrancy-related vulnerabilities
    let has_reentrancy_vuln = result.vulnerabilities.iter().any(|v| 
        v.title.to_lowercase().contains("reentrancy") ||
        v.description.to_lowercase().contains("reentrancy") ||
        v.description.to_lowercase().contains("external call")
    );
    assert!(has_reentrancy_vuln, "Expected reentrancy-related vulnerability, got: {:?}", 
            result.vulnerabilities.iter().map(|v| (&v.title, &v.description)).collect::<Vec<_>>());
...
#[tokio::test]
async fn test_compare_strategies() -> Result<()> {
    ...
    for i in 0..iterations {
        ...
        let result = groth16_verifier.analyze_bytecode(&bytecode).await?;
    ...
    for i in 0..iterations {
        ...
        let start = Instant::now();
        let result = zoda_verifier.analyze_bytecode(&bytecode).await?;
        let elapsed = start.elapsed();
